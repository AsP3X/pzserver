...
async fn finish_ok(state: &AppState, order: &ItemOrder, removed: i32) {
    match order.reference_type.as_str() {
        ...
        "auction_buy_offer_fill" => {
            let _ =
                crate::services::economy::offers::on_fill_moved(state, order.reference_id, removed)
                    .await;
        }
        "vault_move" => {
            let _ =
...
async fn finish_failed(state: &AppState, order: &ItemOrder) {
    match order.reference_type.as_str() {
        ...
        "auction_buy_offer_fill" => {
            let _ =
                crate::services::economy::offers::on_fill_failed(state, order.reference_id).await;
        }
        "vault_move" => {
