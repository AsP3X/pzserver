...
After `make up`, the admin panel is available at `http://localhost:8100`. No firewall changes are needed. The game server is running but not reachable from outside the machine.
