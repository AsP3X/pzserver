...
function Write-Banner {
    ...
    Write-Host "  PZ B42 Server + Panel - First-Time Setup      " -ForegroundColor Cyan
    Write-Host "==============================================  " -ForegroundColor Cyan
    Write-Host ""
}
...
if ((Test-Path ".env") -or (Test-Path "app\.env")) {
    ...
    if ($UseDefaults) {
        Write-Host "  Overwriting (PZ_SETUP_ASSUME_YES=1)..." -ForegroundColor Yellow
    } else {
...
# ══════════════════════════════════════════════════════════════════════
# Interactive prompts (or non-interactive defaults)
# ══════════════════════════════════════════════════════════════════════
Write-Banner
...
if ($UseDefaults) {
    ...
    $ADMIN_PASS_GENERATED = -not [bool]$env:ADMIN_PASSWORD
    $ADMIN_EMAIL = if ($env:ADMIN_EMAIL) { $env:ADMIN_EMAIL } else { "" }

    $PZ_SERVER_NAME = if ($env:PZ_SERVER_NAME) { $env:PZ_SERVER_NAME } else { "ZomboidServer" }
    $PZ_MAX_PLAYERS = if ($env:PZ_MAX_PLAYERS) { $env:PZ_MAX_PLAYERS } else { "16" }
    $PZ_MAX_RAM = if ($env:PZ_MAX_RAM) { $env:PZ_MAX_RAM } else { "8192m" }
...
} while ($branchChoice -notin @("1", "2", "3", "4"))

switch ($branchChoice) {
    "1" { $PZ_STEAM_BRANCH = "public" }
    "2" { $PZ_STEAM_BRANCH = "unstable" }
    "3" { $PZ_STEAM_BRANCH = "legacy41" }
    "4" {
...
} # end interactive prompts branch
...
Write-Host ""
if ($env:PZ_SETUP_ASSUME_YES -eq "1") {
    $proceed = "y"
    Write-Host "  Proceeding automatically (PZ_SETUP_ASSUME_YES=1)..." -ForegroundColor DarkGray
} else {
